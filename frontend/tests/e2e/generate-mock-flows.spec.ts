...
test.describe('Generate API - Mock flows', () => {
  ...
  test("should show rate limit error (429) when TRIGGER_RATE_LIMIT is in input", async ({
    ...
    await page.waitForFunction(() => !!navigator.serviceWorker && !!navigator.serviceWorker.controller).catch(() => {});
    ...
    await expect(page.getByText(/rate limit|try again|Too many requests/i)).toBeVisible({ timeout: 20000 });
  });

...
A comprehensive, example-driven lab to explore Harvest.ai’s frontend technology stack, how it maps to user stories, and how streaming and S2S patterns fit together.
...
## User stories → technology mapping
...
- Mocks: `src/mocks/handlers/generation.ts` (TRIGGER\_\* cases)
...
- Live SSE client (in-app): `src/app/playground/sse/page.tsx` — minimal streaming client
...
- Open `/doc?path=docs/SYSTEM_STATUS.md` (Docs → System Status pages exist in Storybook too).
...
- Use Recharts to show tokens/cost — see suggested example in Tech Stack page.
...
if (!data) return <div style={{ fontFamily: 'monospace' }}>Loading docs status…</div>
  ...
  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
    <div>
      ...
      <div style={{ fontSize: 12, color: "#555" }}>
        mtime: {fmt(devlog?.mtimeIso || devlog?.mtime || devlog?.modifiedAt)}{" "}
      </div>
    </div>
    <div>
      <div style={{ fontWeight: 600 }}>SYSTEM_STATUS.md</div>
      <div style={{ fontSize: 12, color: "#555" }}>
        mtime: {fmt(sys?.mtimeIso || sys?.mtime || sys?.modifiedAt)}{" "}
      </div>
    </div>
    <div>
      <div style={{ fontWeight: 600 }}>EPICS.md</div>
      <div style={{ fontSize: 12, color: "#555" }}>
        mtime: {fmt(epics?.mtimeIso || epics?.mtime || epics?.modifiedAt)}{" "}
      </div>
    </div>
  </div>
</div>

...
async function validatePkg() {
  ...
  const bins = ["jest", "playwright", "next", "storybook"].map((n) => ({
    name: n,
    path: path.join(ROOT, "node_modules", ".bin", n),
  }));
  for (const b of bins) {
    b.ok = await exists(b.path);
  }
...
function summarize(checks, pkg) {
  ...
  for (const c of checks) {
    lines.push(`${c.ok ? "✅" : "❌"} ${path.relative(ROOT, c.path)}`);
    if (!c.ok) fail.push(c);
  }
  lines.push("---");
  lines.push("Scripts:");
  for (const s of pkg.scripts) {
    ...
    if (!s.ok) fail.push({ name: "script:" + s.name });
  }
  lines.push("Bins:");
  for (const b of pkg.bins) {
    lines.push(`${b.ok ? "✅" : "❌"} node_modules/.bin/${b.name}`);
    if (!b.ok) fail.push({ name: "bin:" + b.name });
  }

...
export function EpicsSpecDoc() {
  ...
  useEffect(() => {
    ...
    function toHtml(md){
      ...
      const chunks=out.map(p=>p.k==='c'?`<pre style="background:#0b0f19;color:#e6edf3;padding:12px;border-radius:6px;overflow:auto"><code class="language-${esc(p.l)}">${esc(p.v)}</code></pre>`:p.v
  ...
  return <div style={{border:'1px solid #eee',borderRadius:6,padding:12,background:'#fff'}} dangerouslySetInnerHTML={{__html:html}} />
}

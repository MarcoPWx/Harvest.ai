...
function log(...args) {
  console.log("[docs:refresh]", ...args);
}
function warn(...args) {
  console.warn("[docs:refresh]", ...args);
}
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === "--context-file" && argv[i + 1]) {
      out.contextFile = argv[++i];
      continue;
    }
    if (a.startsWith("--context-file=")) {
      out.contextFile = a.slice("--context-file=".length);
      continue;
    }
    if (a === "--max-commits" && argv[i + 1]) {
      out.maxCommits = Math.max(1, parseInt(argv[++i], 10) || 5);
      continue;
    }
    if (a.startsWith("--max-commits=")) {
      out.maxCommits = Math.max(1, parseInt(a.slice("--max-commits=".length), 10) || 5);
      continue;
    }
  }
  return out;
...
async function readContextFromArgsEnvClipboard(args) {
  ...
  if (args.contextFile) {
    try {
      const t = await readFile(path.resolve(args.contextFile), "utf8");
